...
const analyzeWeatherConditions = (weatherData) => {
  ...
  } else if (current.wind_speed_10m >= VESSEL_THRESHOLDS.fishing.wind_speed_kmh) {
    issues.push({
      type: 'wind',
      severity: current.wind_speed_10m >= 45 ? SEVERITY.WARNING : SEVERITY.CAUTION,
  ...
  if (current.wind_gusts_10m >= VESSEL_THRESHOLDS.commercial.wind_gust_kmh) {
    issues.push({
      type: 'gust',
      severity: SEVERITY.DANGER,
      message: `Dangerous wind gusts: ${Math.round(current.wind_gusts_10m)} km/h`,
    });
    maxSeverity = SEVERITY.DANGER;
  } else if (current.wind_gusts_10m >= VESSEL_THRESHOLDS.fishing.wind_gust_kmh) {
    issues.push({
      type: 'gust',
  ...
  if (current.precipitation >= VESSEL_THRESHOLDS.commercial.precipitation_mm_h) {
    issues.push({
      type: 'precipitation',
      severity: SEVERITY.WARNING,
  ...
  } else if (current.precipitation >= VESSEL_THRESHOLDS.fishing.precipitation_mm_h) {
    issues.push({
      type: 'precipitation',
...
const generateRecommendations = (weatherAnalysis, waveAnalysis) => {
  ...
  const combinedSeverity = 
    weatherAnalysis?.severity === SEVERITY.DANGER || waveAnalysis?.severity === SEVERITY.DANGER
      ? SEVERITY.DANGER
      : weatherAnalysis?.severity === SEVERITY.WARNING || waveAnalysis?.severity === SEVERITY.WARNING
      ? SEVERITY.WARNING
      : weatherAnalysis?.severity === SEVERITY.CAUTION || waveAnalysis?.severity === SEVERITY.CAUTION
      ? SEVERITY.CAUTION
      : SEVERITY.SAFE;

  // Fishing boat recommendations
  if (combinedSeverity === SEVERITY.DANGER) {
    recommendations.fishing.push('⛔ DO NOT SAIL - Conditions are dangerous for fishing vessels');
    ...
    recommendations.fishing.push('⚠️ NOT RECOMMENDED - Conditions are hazardous for small vessels');
    recommendations.fishing.push('Only experienced crews with proper equipment should consider sailing');
    recommendations.fishing.push('Stay close to shore and monitor weather closely');
  ...
  if (combinedSeverity === SEVERITY.DANGER) {
    recommendations.commercial.push('⚠️ EXTREME CAUTION - Hazardous conditions present');
    ...
    recommendations.commercial.push('⚠️ PROCEED WITH CAUTION - Challenging conditions');
    ...
    recommendations.commercial.push('⚠️ MINOR CAUTION - Some challenging conditions');
  ...
  if (weatherAnalysis?.issues.length > 0 || waveAnalysis?.issues.length > 0) {
    ...
    recommendations.general.push('Ensure communication equipment is operational');
    recommendations.general.push('Have emergency contacts readily available');
  }

  if (waveAnalysis && waveAnalysis.waveHeight >= 1.5) {
    recommendations.general.push('Expect rough seas - secure all cargo and equipment');
  }
...
export const analyzeLocation = async (lat, lng, locationName) => {
  try {
    ...
    const recommendations = generateRecommendations(weatherAnalysis, waveAnalysis);
...
export const analyzeMultipleLocations = async (locations) => {
  const results = await Promise.all(
    locations.map(loc => 
      analyzeLocation(loc.latitude, loc.longitude, loc.port_name || loc.location)
    )
  );
